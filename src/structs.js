...
const PublicKeyEcc = (validation) => {
  return {
    fromByteBuffer(b) {
      ...
      return PublicKey.fromBuffer(pubbuf)
        .toString();
    },
    ...
    appendByteBuffer(b, value) {
      ...
      // }
      const buf = PublicKey.fromStringOrThrow(value)
        .toBuffer();
  ...
  'setcode.code.fromObject': ({ object, result }) => {
    try {
      ...
      if (/^\s*\(module/.test(code)) {
        ...
        const wasm = Buffer.from(binaryen.parseText(code)
          .emitBinary());
        result.code = wasm;
      } else {
...
const actionDataOverride = (structLookup, forceActionDataHex) => ({
  ...
  'action.data.toObject': ({ fields, object, result, config }) => {
    ...
    if (forceActionDataHex) {
      ...
      result.data = b2.copy(0, b2.offset)
        .toString('hex');
