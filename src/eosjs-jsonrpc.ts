...
export class JsonRpc implements AuthorityProvider, AbiProvider {
    ...
    public async abi_bin_to_json(code: string, action: string, binargs: string): Promise<AbiBinToJsonResult> {
        return await this.fetch('/v1/chain/abi_bin_to_json', { code, action, binargs });
    }

    public async abi_json_to_bin(code: string, action: string, args: any[]): Promise<AbiJsonToBinResult> {
