...
describe('offline', () => {
  const headers = {
    expiration: new Date().toISOString()
      .split('.')[0], // Don't use `new Date` in production
    ref_block_num: 1,
...
describe('transactions', () => {
  ...
  it('keyProvider multiple private keys (get_required_keys)', () => {
    ...
    return eos.transfer('inita', 'initb', '1.2740 SYS', '', false)
      .then(tr => {
        assert.equal(tr.transaction.signatures.length, 1);
        assert.equal(typeof tr.transaction.signatures[0], 'string');
      });
  ...
  it('keyProvider public keys then private key', () => {
    ...
    return eos.transfer('inita', 'initb', '9.0000 SYS', '', false)
      .then(tr => {
        assert.equal(tr.transaction.signatures.length, 1);
        assert.equal(typeof tr.transaction.signatures[0], 'string');
      });
  ...
  it('keyProvider return Promise', () => {
    const eos = Eos({
      keyProvider: new Promise(resolve => {
        resolve(wif);
      })
    });
  ...
  it('signProvider', () => {
    const customSignProvider = ({ buf, sign, transaction }) => {
      ...
      return eos.getRequiredKeys(transaction, pubkeys)
        .then(res => {
  ...
  it('transfer custom authorization sorting (no broadcast)', () => {
    ...
    return eos.transfer('inita', 'initb', '1.0000 SYS', '',
      {
        authorization: ['initb@owner', 'inita@owner'],
        broadcast: false
      }
    )
      .then(({ transaction }) => {
  ...
  it('transfer (no broadcast, no sign)', () => {
    ...
    return eos.transfer('inita', 'initb', '1.0000 SYS', '', opts)
      .then(tr =>
  ...
  it('action to contract (contract tr nesting)', function () {
    ...
    return tn.contract('eosio.token')
      .then(eosio_token => {
...
const randomName = () => {
  const name = String(Math.round(Math.random() * 1000000000))
    .replace(/[0,6-9]/g, '');
  return 'a' + name + '111222333444'.substring(0, 11 - name.length); // always 12 in length
};

const randomAsset = () =>
  ecc.sha256(String(Math.random()))
    .toUpperCase()
    .replace(/[^A-Z]/g, '')
    .substring(0, 7);
