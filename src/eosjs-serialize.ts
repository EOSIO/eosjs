 ...
 */
export type Anyvar = null | string | number | Uint8Array | Anyvar[] | { type: string; value: any } | {}
...
const anyvarDefs = {
  null_t: {
    index: 0,
    useShortForm: true,
    type: { name: 'null_t', serialize: serializeNull, deserialize: deserializeNull },
  },
  ...
  any_object: {
    index: 13,
    useShortForm: true,
    type: { name: 'any_object', serialize: serializeAnyObject, deserialize: deserializeAnyObject },
  },
  any_array: {
    index: 14,
    useShortForm: true,
    type: { name: 'any_array', serialize: serializeAnyArray, deserialize: deserializeAnyArray },
  },
...
function serializeNull(buffer: SerialBuffer, anyvar: Anyvar) {}

function deserializeNull(buffer: SerialBuffer, state?: SerializerState) {}

export function serializeAnyvar(buffer: SerialBuffer, anyvar: Anyvar) {
  let def: AnyvarDef
  let value: any
  if (anyvar === null) [def, value] = [anyvarDefs.null_t, anyvar]
  else if (typeof anyvar === 'string') [def, value] = [anyvarDefs.string, anyvar]
  else if (typeof anyvar === 'number') [def, value] = [anyvarDefs.int32, anyvar]
  else if (anyvar instanceof Uint8Array) [def, value] = [anyvarDefs.bytes, anyvar]
  else if (Array.isArray(anyvar)) [def, value] = [anyvarDefs.any_array, anyvar]
  else if (
  ...
  else [def, value] = [anyvarDefs.any_object, anyvar]
...
export function deserializeAnyvar(buffer: SerialBuffer, state?: SerializerState) {
  ...
  if (defIndex >= anyvarDefsByIndex.length) throw new Error('Tried to deserialize unknown anyvar type')
  const def = anyvarDefsByIndex[defIndex]
  const value = def.type.deserialize(buffer, state)
  if ((state && (state.options as any).useShortForm) || def.useShortForm) return value
  else return { type: def.type.name, value }
}
...
export function serializeAnyArray(buffer: SerialBuffer, arr: Anyvar[]) {
  ...
  for (let x of arr) serializeAnyvar(buffer, x)
}

export function deserializeAnyArray(buffer: SerialBuffer, state?: SerializerState) {
  const len = buffer.getVaruint32()
  const result = []
  for (let i = 0; i < len; ++i) result.push(deserializeAnyvar(buffer, state))
  return result
}
export function serializeQuery(buffer: SerialBuffer, query: Query) {
  ...
  let arg: Anyvar
  let filter: Query[]
  if (typeof query === 'string') method = query
  else if (Array.isArray(query) && query.length === 2) [method, filter] = query
  else if (Array.isArray(query) && query.length === 3) [method, arg, filter] = query
  else [method, arg, filter] = [query.method, query.arg, query.filter]
  ...
  if (filter === undefined) {
    ...
    for (let q of filter) serializeQuery(buffer, q)
  }
}
