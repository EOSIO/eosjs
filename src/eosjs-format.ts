...
export function ULong(value: Long | number | string, unsigned = true, radix = 10) {
...
export function isName(str: string, err: any) {
  try {
    encodeName(str)
    return true
  } catch(error) {
...
*/
export function encodeName(name: string, littleEndian: boolean = true) {
...
/**
  ...
  @return {string}
*/
export function decodeName(value: Long | string | number, littleEndian: boolean = true) {
  value = ULong(value) as Long
...
export function encodeNameHex(name: string) {
  return Long.fromString(encodeName(name), true).toString(16)
}

export function decodeNameHex(hex: string, littleEndian: boolean = true) {
...
/**
  ...
  @return {string} value
*/
export function DecimalString(value: string | number) {
...
*/
export function DecimalPad(num: number | string, precision: number) {
...
export function DecimalImply(value: number | string, precision: number) {
...
*/
export function DecimalUnimply(value: string | number, precision: number) {
  assert(value != null, 'value is required')
  value = value === 'object' && value.toString ? value.toString() : String(value)
  const neg = /^-/.test(value)
  if(neg) {
...
*/
function printAsset(obj: {amount: string, precision: number, symbol: string, contract: string}) {
  assert.equal(typeof obj.symbol, 'string', 'symbol is a required string')

  if(obj.amount != null && obj.precision != null) {
    obj.amount = DecimalPad(obj.amount, obj.precision)
  }

  const join = (e1: string | number, e2: string) => e1 == null ? '' : e2 == null ? '' : e1 + e2

  if(obj.amount != null) {
    // the amount contains the precision
    return join(obj.amount, ' ') + obj.symbol + join('@', obj.contract)
  }

  return join(obj.precision, ',') + obj.symbol + join('@', obj.contract)
}
